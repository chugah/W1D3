...
puts grab_sin("please don't share this: 234-604-142").to_s 
​
puts "grab_sin returns nil if it doesn't have a SIN"
puts grab_sin("please confirm your identity: XXX-XXX-142").inspect
​
​
# Return all of the SINs from a string, not just one.
def grab_all_sins(string)
  if string =~ (/^\d{3}-\d{3}-\d{3}\W\s\d{3}-\d{3}-\d{3}\W\s\d{3}-\d{3}-\d{3}$/)
    return string.split(", ")
  end
...
p grab_all_sins("234-604-142, 350-802-074, 013-630-876") 
​
​
puts "grab_all_sins returns an empty Array if it doesn't have any SINs"
puts grab_all_sins("please confirm your identity: XXX-XXX-142") == nil 
​
​
